...
export default function ToolSMARTNova() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const { entry, error: createError } = await createToolEntry("smart", "Nova Análise", {});
    ...
    <main style={{ backgroundColor: "var(--cream)", minHeight: "100vh" }}>
      <header className="text-white py-8 px-6" style={{ backgroundColor: "var(--navy)" }}>
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <Link href="/ferramenta/smart" style={{ color: "rgba(255,255,255,0.7)" }}>
            ← Voltar
          </Link>
        </div>
      </header>
