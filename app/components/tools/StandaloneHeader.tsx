...
export function StandaloneHeader({ icon, name, badge, toolSlug }: Props) {
  const [copied, setCopied] = useState(false);
  const [hasPlus, setHasPlus] = useState(false);

  useEffect(() => {
    const match = document.cookie.match(/(?:^|;\s*)plus_tools=([^;]*)/);
    if (match) setHasPlus(match[1].split(",").includes(toolSlug));
  }, [toolSlug]);
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <Link href="/" style={{ textDecoration: "none" }}>
            <div style={{
              ...
              fontSize: 15, color: "var(--navy)", fontWeight: 700, letterSpacing: "-0.5px",
            }}>MC</div>
          </Link>
